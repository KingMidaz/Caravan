...
class MyOpenGLWidget(QOpenGLWidget):
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if not self.mouseDown and self.player1 is not None:
            ...
            if self.highlightedCard is not None:
                    self.highlightedCard.topleft = QPoint(self.highlightedCard.topleft.x(), self.highlightedCard.topleft.y() - self.highlightedCard.height/4)
            self.update()
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        #if self.player1.drawpile.isMouseOver(event.pos()):
        #       pass
                ...
                if p is not None and self.selectedCard is not None:
                    ...
                    for k, i in zip(p.lanes.keys(), range(0, len(p.lanes.keys()))):
                        if (QRect(QPoint(i*(self.width())/5 + self.width()*2/20, self.height()/2-10), 
                                QPoint(i*(self.width())/5 + self.width()*4/20, self.height()/2-10 - 20)).contains(event.pos())):
    ...
    def adjustUIScale(self):
        for p in [self.player1, self.player2]:
            if p is not None: 
                if p.flip:
                    p.hand.topleft = QPoint(self.width()*9/10 - len(p.getHand().cards) * cards.Card.width/2, self.reflectY(self.height()*3/5) - cards.Card.height)
                    p.hand.flipHand(True)
                    p.drawpile.topleft = QPoint(self.width() - self.width()/12, self.reflectY(self.height() - cards.Card.height - self.height()/30) - cards.Card.height)
                    for k, i in zip(p.lanes.keys(), range(0,3)):
                        p.lanes[k].playzone = QRect(QPoint((i*(self.width())/5), self.reflectY((self.height()/2+10) + self.height()/3)), QPoint(((i+1)*(self.width())/5), self.reflectY(self.height()/2+10)))
                else:
                    p.hand.topleft = QPoint(self.width()*9/10 - len(p.getHand().cards) * cards.Card.width/2, self.height()*3/5)
                    p.drawpile.topleft = QPoint(self.width() - self.width()/12, self.height() - cards.Card.height - self.height()/30)
                    for k, i in zip(p.lanes.keys(), range(0,3)):
                        p.lanes[k].playzone = QRect(QPoint((i*(self.width())/5), self.height()/2+10), QPoint(((i+1)*(self.width())/5), (self.height()/2+10) + self.height()/3))
                    p.discardzone = QRect(QPoint(self.width()*9/10 - len(p.getHand().cards) * cards.Card.width/2, self.height()*3/5 + cards.Card.height * 1.2), 
                                          QPoint(self.width()*9/10 - len(p.getHand().cards) * cards.Card.width/2 + len(p.getHand().cards)*cards.Card.width/2, self.height()*3/5 + cards.Card.height * 2.2))
                p.hand.reindex()
    ...
    def paintPlayers(self, painter: QPainter):
        for p in [self.player1, self.player2]:
            ...
            for c, i in zip(p.getDrawPile().cards[:min(10, len(p.getDrawPile().cards))], range(0, min(10, len(p.getDrawPile().cards)))):
            ...
            if p is not None: 
                for k in p.lanes.keys():
                    for c, i in zip(p.lanes[k].cards, range(0, len(p.lanes[k].cards))):
                        if p.flip:
                            c.topleft = QPoint(p.lanes[k].playzone.bottomLeft().x() + self.width()*5/50 + i%2*4 + i%3*3, p.lanes[k].playzone.bottomLeft().y() - (i+1)*c.height + c.height*i*4/5 - 40)
                        else:
                            c.topleft = QPoint(p.lanes[k].playzone.topLeft().x() + self.width()*5/50 + i%2*4 + i%3*3, p.lanes[k].playzone.topLeft().y() + i*c.height - c.height*i*4/5)
                        self.paintCard(painter, c, p.flip)

                    painter.setFont(self.laneFont)
                    if not p.flip:
                        painter.drawText(QPoint(p.lanes[k].playzone.topLeft().x() + self.width()/25, p.lanes[k].playzone.topLeft().y()), str(p.lanes[k].score))
                    else:
                        painter.drawText(QPoint(p.lanes[k].playzone.bottomLeft().x() + self.width()/25, p.lanes[k].playzone.bottomLeft().y()), str(p.lanes[k].score))
